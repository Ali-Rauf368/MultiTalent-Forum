...
console.log(chalk.blue("Please select from the following options:\n") + 
    chalk.green("1. Quiz\n") + 
    chalk.green("2. Guessing Number\n") + 
    chalk.green("3. Thrilling Games\n"));

const option1 = "1";
const option2 = "2";
const option3 = "3";

const choice = prompt(chalk.yellow('Enter your choice Number as 1 or 2 or 3: '));

let prizeMoney = 0;

/*Quiz Section */
if (choice === option1) {
    console.log(chalk.magenta("Welcome to the Quiz Section!\n"));
    console.log(chalk.blue("Please select from the following options:\n") + 
        chalk.green("1. Quiz Related to Books\n") + 
        chalk.green("2. General Knowledge\n"));

    const subChoice = prompt(chalk.yellow('Enter your choice Number as 1 or 2: '));
            ...
            for (let i = 0; i < shuffledQuestions.length; i++) {
                ...
                if (shuffledQuestions[i].type === "multiple") {
                    ...
                    for (let j = 0; j < shuffledQuestions[i].options.length; j++) {
                        console.log(chalk.blue(`${j + 1}. ${shuffledQuestions[i].options[j]}`));
                    }

                    let answerIndex = parseInt(prompt(chalk.yellow('Enter your answer (number): '))) - 1;
                    ...
                    if (answer.toLowerCase() === shuffledQuestions[i].answer.toLowerCase()) {
                        ...
                        if (wrongAnswers === maxWrongAnswers) {
                            console.log(chalk.red("You have reached the maximum number of wrong answers. Better luck next time!"));
                            break;
                        }
                    }
                } else if (shuffledQuestions[i].type === "truefalse") {
                    let answer = prompt(chalk.yellow('Enter your answer (true/false): '));
    ...
    /*General Knowledge */
        ...
        function quiz() {
            ...
            for (let i = 0; i < shuffledQuestions.length; i++) {
                console.log(chalk.cyan(`\nQuestion ${i + 1}: ${shuffledQuestions[i].question}`));
                let answer = prompt(chalk.yellow('Enter your answer: '));
...
/* Guessing Number*/
    ...
    while (counter < 3 && !guessedCorrectly) {
        ...
        if (parseInt(userGuess) === randomNumber) {
            console.log(chalk.green("Congratulations! You guessed the correct number. You win $200.\n"));
            prizeMoney = 200;
    ...
    if (!guessedCorrectly) {
        console.log(chalk.red(`Sorry, you've used all attempts. The correct number was ${randomNumber}. Better luck next time!\n`));
    }
}

/* Thrilling Games */
else if (choice === option3) {
    console.log(chalk.magenta("Welcome to the Thrilling Games!\n"));
    console.log(chalk.blue("Please select from the following options:"));
   
    console.log(chalk.green("1. Fighting Game"));
    console.log(chalk.green("2. Tic-Tac-Toe"));
...
/* Free Style Fighting */
    ...
    if (subChoice === option1) {
        function fight() {
            ...
            while (power1 > 0 && power2 > 0) {
                if (turn === 1) {
                    let move = prompt(chalk.yellow(`${fighter1}, enter your move (jab/hammerfist/kick/choke): `));
                    if (moves[move] !== undefined) {
                        power2 -= moves[move];
                        console.log(chalk.green(`${fighter1} uses ${move} on ${fighter2} and ${fighter2}'s new power is ${power2}`));
                        turn = 2; 
                    } else {
                        console.log(chalk.red("Invalid move, try again."));
                    }
                } else {
                    let move = prompt(chalk.yellow(`${fighter2}, enter your move (jab/hammerfist/kick/choke): `));
                    if (moves[move] !== undefined) {
                        power1 -= moves[move];
                        console.log(chalk.green(`${fighter2} uses ${move} on ${fighter1} and ${fighter1}'s new power is ${power1}`));
                        turn = 1; 
                    } else {
                        console.log(chalk.red("Invalid move, try again."));
                    }
    ...
    /* Tic-Tac-Toe Game */
        ...
        let columns = parseInt(prompt(chalk.yellow("Enter the number of columns:")));
        ...
        while (cellsFilled < totalCells) {
            ...
            if (turn === 1) {
                let rowIndex = parseInt(prompt(chalk.yellow(`${player1Name}, enter row index (0 to ${rows - 1}):`)));
                let columnIndex = parseInt(prompt(chalk.yellow(`${player1Name}, enter column index (0 to ${columns - 1}):`)));

                if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns && matrix[rowIndex][columnIndex] === 0) {
                    let value = prompt(chalk.yellow(`${player1Name}, enter value for element at row ${rowIndex}, column ${columnIndex} (enter 'X'):`));
                    if (value === "X") {
                    ...
                    console.log(chalk.red("Invalid input or cell already filled. Please try again."));
                }
            } else if (turn === 2) {
                let rowIndex = parseInt(prompt(chalk.yellow(`${player2Name}, enter row index (0 to ${rows - 1}):`)));
                let columnIndex = parseInt(prompt(chalk.yellow(`${player2Name}, enter column index (0 to ${columns - 1}):`)));

                if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns && matrix[rowIndex][columnIndex] === 0) {
                    let value = prompt(chalk.yellow(`${player2Name}, enter value for element at row ${rowIndex}, column ${columnIndex} (enter '✓'):`));
                    if (value === "✓") {
                        matrix[rowIndex][columnIndex] = value;
                        cellsFilled++;
                        if (checkWin('✓')) {
...
function shoppingCart() {
    console.log(chalk.cyan("Congratulations! Welcome to Our Online Shop. You can buy different items like jeans, shirts, hats, shoes, etc."));
    const products = [
    ...
    while (prizeMoney > 0) {
        let itemType = prompt(chalk.yellow('Enter the type of item you want to buy: ')).toLowerCase();
        let itemColor = prompt(chalk.yellow('Enter the color of the item you want to buy: ')).toLowerCase();
        
        let itemQuantity;
        while (true) {
            itemQuantity = parseInt(prompt(chalk.yellow('Enter the quantity you want to buy: ')), 10);
            if (isNaN(itemQuantity) || itemQuantity <= 0) {
                console.log(chalk.red('Invalid quantity. Please enter a positive number.'));
            } else {
                break;
            }
        }
        ...
        if (filteredProducts.length > 0) {
            ...
            if (totalPrice <= prizeMoney) {
                console.log(chalk.green(`You have selected ${itemQuantity} ${itemType}(s) of color ${itemColor} with a total price of $${totalPrice.toFixed(2)}`));
                ...
                console.log(chalk.red(`The total price of the selected products exceeds your prize money. You have $${prizeMoney.toFixed(2)} left.`));
            }
        } else {
            console.log(chalk.red(`No products found for type: ${itemType} and color: ${itemColor}`));
        }

        if (prizeMoney > 0) {
            let continueShopping = prompt(chalk.yellow('Do you want to continue shopping? (yes/no): ')).toLowerCase();
    ...
    console.log(chalk.green(`Remaining Prize Money: $${prizeMoney.toFixed(2)}`));
    console.log(chalk.green(`Total Money Utilized: $${(200 - prizeMoney).toFixed(2)}`));
    console.log(chalk.cyan('Purchased Items:'));
    purchasedItems.forEach(item => {
        console.log(chalk.green(`- ${item.type} (${item.color}) - $${item.price.toFixed(2)}`));
    });
}
